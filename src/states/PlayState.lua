...
function PlayState:init()
    ...
    --Update Timer : points representation vars
    self.seconds = 0
    self.secondsY = 108
...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        elseif love.keyboard.wasPressed('right') then
           ...
           -- Update: Mouse Input: Cursor when enter board
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            --Update Timer: extends the timer by 1 second per tile in a match
            self.timer = self.timer + #match
            self.seconds = self.seconds + #match
        end

        --Update Timer: wait 1 second, then move up and fade out points then reset the vars
